...
const RoomReportComponent: React.FC = () => {
  ...
  const filteredData = useMemo(() => {
    return allRooms
      .filter(
        (item) => item.state === PENDING_STATE || item.state === APPROVED_STATE
      )
      .filter((item) => {
  ...
  const updateRoomData = async (roomData: RoomRecord, newState: RoomState) => {
    if (roomData.state !== PENDING_STATE) {
      toast.warn(
        `Phòng ${roomData.roomName} đang ở trạng thái ${roomData.state} và không thể duyệt.`
      );
      return;
    }
  ...
  const {
    ...
    fileName: `BC_CSVC_BaoCaoTongHop_${selectedYear}_${dayjs().format(
      "YYYYMMDD"
    )}`,
  });
  ...
  const handleExportRoomDetails = (record: RoomRecord) => {
    ...
    if (dataToExport.length === 0) {
      toast.info(
        `Phòng ${record.roomName} không có thiết bị hỏng hoặc thiếu nào cần báo cáo.`
      );
      return;
    }
    exportDetailsToExcel({
    ...
    () => [
      ...
      },
      {
        ...
        render: (text, record) => (
          <Button
            type="link"
            onClick={() =>
              (window as any).navigate(`/rooms/view/${record._id}`)
            }
          >
            {text}
          </Button>
      ...
      },
      {
        ...
        render: (state: RoomState) => (
          <Tag color={getTagColor(state)}>{state}</Tag>
        ),
      },
      {
        title: "Hành động",
