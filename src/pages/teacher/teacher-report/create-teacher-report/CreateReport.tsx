...
function CreateReport() {
    ...
    const columns = useMemo<ColumnsType<IScheduleRow>>(() => {
        ...
        const dayColumns: ColumnsType<IScheduleRow> =
            scheduleData.scheduleDays.map((day, dayIndex) => ({
                title: (
                ...
                render: (
                    ...
                    const isHolidayAndEmpty = day.isHoliday && day.activities.length === 0;
                                ...
                                >
                                    {isEditing ? (
                                        <Input.TextArea
                                            ...
                                            onKeyDown={(e) => handleNoteKeyDown(e, dayIndex, rowIndex)}
                                            autoSize={{ minRows: 2 }}
        ...
        >
            <Space style={{ marginBottom: 16 }}>
                ...
                <Space.Compact>
                    ...
                    />
                    <Button
                        style={{ width: 'auto', minWidth: 120, cursor: 'default' }}
                    >
                        {weekDateRange}
                        {(scheduleData?.scheduleDays?.length ?? 0) > 0 && (
                            <Text type="secondary" style={{ marginLeft: 8 }}>
                                {scheduleData && scheduleData.scheduleDays.length > 0 &&
                                    `(Tuần ${getWeekOfMonth(scheduleData.scheduleDays[0].date)} trong tháng)`}
                            </Text>
                        )}
                    </Button>
            ...
            <Row justify="end" style={{ marginBottom: 8 }}>
                <Popover
                    ...
                    content={
                        <div>
                            • Nhấp vào ô hoạt động <Tag color="green" icon={<EditOutlined />}>Bình thường</Tag> để nhập nội dung bài học.
                            <br />• Dùng <code>-</code> để gạch đầu dòng mỗi ý. (Ví dụ: <code>- Hoạt động A</code>)
                            <br />• Nhấn <kbd>Shift</kbd> + <kbd>Enter</kbd> hoặc Enter để xuống dòng mới trong cùng một hoạt động.
                        </div>
                    }
                    trigger="click"
                >
