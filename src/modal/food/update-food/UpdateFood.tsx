...
const UpdateFoodPage: React.FC = () => {
  ...
  const [currentTotalCalories, setCurrentTotalCalories] = useState(
    foodDetailFromState?.totalCalories || 0
  );
  ...
  useEffect(() => {
    if (foodDetailFromState) {
      setFoodDetail(foodDetailFromState);
    }
  }, [foodDetailFromState]);

  const calculateAndSetTotalNutrients = useCallback(
    (ingredients: Ingredient[]) => {
  ...
  const fetchUpdatedFoodDetails = useCallback(async () => {
    ...
    try {
      const response: AICalculateResponse =
        await foodApis.calculateFoodNutrients(id);
      const updatedFood: FoodRecord = response.ai_output[0];

      setFoodDetail(updatedFood);
      navigate(`${constants.APP_PREFIX}/foods/view/${id}`, {
        state: {
          foodDetail: updatedFood as FoodRecord,
        },
      });
    } catch (error) {
  ...
  useEffect(() => {
    if (foodDetail) {
      ...
      calculateAndSetTotalNutrients(foodDetail?.ingredients || []);
      setLoading(false);
    }
  }, [id, foodDetail, form, calculateAndSetTotalNutrients]);

  const onFormValuesChange = (
    changedValues: any,
    allValues: UpdateFoodFormValues
  ) => {
    if (
      changedValues.ingredients ||
      changedValues.foodName ||
      changedValues.ageGroup ||
      changedValues.active
    ) {
  ...
  if (!foodDetail) {
    return (
      <div style={{ padding: "24px" }}>
        ...
        <Card>
          <Text type="danger">
            Lỗi: Không có dữ liệu món ăn cho ID: {id}. Vui lòng quay lại màn
            Danh sách để chọn lại.
          </Text>
        </Card>
      </div>
    );
        ...
        >
          <Row gutter={24}>
            <Col span={8}>
              ...
              >
                <Input
                  prefix={<TagOutlined />}
                  placeholder="Ví dụ: Cháo gà nấm hương"
                />
              </Form.Item>
            </Col>
            <Col span={8}>
              <Form.Item
                ...
                rules={[
                  { required: true, message: "Vui lòng chọn nhóm tuổi!" },
                ]}
              >
                <Select
          ...
          <Divider orientation="left">
            <Text strong style={{ color: "#fa541c" }}>
              <DashboardOutlined /> Tổng Quan Dinh Dưỡng
            </Text>
          </Divider>
          ...
          <Row gutter={16} style={{ marginBottom: 20, textAlign: "center" }}>
            <Col span={6}>
              ...
              >
                <Text style={{ color: "#ff7a45", display: "block" }}>
                  TỔNG CALO
                </Text>
                <Text strong style={{ color: "#ff4d4f", fontSize: "1.8em" }}>
                  {currentTotalCalories.toLocaleString("vi-VN", {
            ...
            <Col span={6}>
              <Card size="small" style={{ backgroundColor: "#f9f0ff" }}>
                <Text style={{ color: "#722ed1", display: "block" }}>
                  Protein (g)
                </Text>
                <Text strong style={{ color: "#722ed1", fontSize: "1.8em" }}>
                  {currentTotalProtein.toLocaleString("vi-VN", {
                    maximumFractionDigits: 2,
                  })}
                </Text>
              </Card>
            ...
            <Col span={6}>
              <Card size="small" style={{ backgroundColor: "#fffbe6" }}>
                ...
                <Text strong style={{ color: "#faad14", fontSize: "1.8em" }}>
                  {currentTotalLipid.toLocaleString("vi-VN", {
                    maximumFractionDigits: 2,
                  })}
                </Text>
              </Card>
            ...
            <Col span={6}>
              <Card size="small" style={{ backgroundColor: "#f6ffed" }}>
                <Text style={{ color: "#52c41a", display: "block" }}>
                  Carb (g)
                </Text>
                <Text strong style={{ color: "#52c41a", fontSize: "1.8em" }}>
                  {currentTotalCarb.toLocaleString("vi-VN", {
                    maximumFractionDigits: 2,
                  })}
                </Text>
              </Card>
          ...
          <Divider orientation="left">
            <Text strong style={{ color: "#1890ff" }}>
              <TagOutlined /> Chi Tiết Nguyên Liệu
            </Text>
          </Divider>
              ...
              <>
                    ...
                    >
                      ...
                      <Row gutter={16}>
                        <Col span={6}>
                          ...
                          >
                            <Input
                              value={ingredient?.calories?.toFixed(2) || "0.00"}
                              disabled
                        ...
                        <Col span={6}>
                          ...
                          >
                            <Input
                              value={ingredient?.protein?.toFixed(2) || "0.00"}
                              disabled
                            />
                          </Form.Item>
                        </Col>
                        <Col span={6}>
                          ...
                          >
                            <Input
                              value={ingredient?.lipid?.toFixed(2) || "0.00"}
                              disabled
                            />
                          </Form.Item>
                        </Col>
                        <Col span={6}>
                          ...
                          >
                            <Input
                              value={ingredient?.carb?.toFixed(2) || "0.00"}
                              disabled
                            />
                          </Form.Item>
                        </Col>
                ...
                <Form.Item>
                  <Button
                    type="dashed"
                    onClick={() =>
                      add({ name: "", gram: 0, unit: "g" } as Ingredient)
                    }
                    block
